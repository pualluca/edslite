...
abstract class LocationShortcutWidgetConfigActivityBase : SettingsBaseActivity() {
    class MainFragment : PropertiesFragmentBase() {
        inner class TargetPathPropertyEditor internal constructor() :
        ...
        override fun createProperties() {
            _propertiesView.addProperty(object : TextPropertyEditor(
                this, R.string.enter_widget_title, 0,
                tag
            ) {
                override fun loadText(): String {
        ...
        private fun initWidgetFields(title: String, path: String) {
            ...
            val target = LocationsManager.getLocationsManager(
                activity
            ).getDefaultLocationFromPath(path)
            val info = LocationShortcutWidgetInfo()
            info.widgetTitle = title
            info.locationUriString = target.locationUri.toString()
            UserSettings.getSettings(context).setLocationShortcutWidgetInfo(widgetId, info)
        ...
        private val widgetId: Int
            get() = activity.intent.getIntExtra(
                AppWidgetManager.EXTRA_APPWIDGET_ID,
