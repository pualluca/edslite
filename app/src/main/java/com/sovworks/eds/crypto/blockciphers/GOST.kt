...
class GOST : BlockCipherNative {
    @Throws(EncryptionEngineException::class)
    override fun init(key: ByteArray) {
        _contextPtr = initContext(key)
        if (_contextPtr == 0L) throw EncryptionEngineException("GOST context initialization failed")
    }
    ...
    fun useTestSubstMask(`val`: Boolean) {
        _useTestSubstMask = `val`
    }

    private var _contextPtr: Long = 0
    private var _useTestSubstMask = false
